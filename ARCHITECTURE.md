...
### `model_loader.py`
- **Purpose**: Neural network model management
- **Responsibilities**:
...
### `trainer.py`
- **Purpose**: Self-play training loop
- **Responsibilities**:
  - Generate training data (self-play)
  - Supervised learning on heuristic labels
  - Integration with stable-baselines3 (optional)
  - Model checkpointing
...
function minimax_ab(state, depth, alpha, beta, maximizing):
    if depth == 0 or is_terminal(state):
        return None, value_fn(state)  # RL model or heuristic
...
1. Extend `trainer.py` with new training loop
2. Implement value function extraction from policy
...
### Model Deployment
- Model must be saved as `.pt` file
- Include in VM image
- Load with `ModelLoader(model_path, device="cpu")`
